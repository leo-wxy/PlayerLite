...
- [ ] 3.8 收口顶部 compact top bar 与 sticky tabs 的职责分离，避免双重状态栏占位、标题与 tab 互相覆盖，并确保渐变与标题淡入由连续滚动进度驱动而非硬切
- [ ] 3.9 收口 tab 切换滚动语义：未吸顶前切换 tab 不改变外层滚动位置，已吸顶或已进入 tab 内容区后切换到新 tab 首项，而不是仅滚到 sticky header
