...
internal class PlaybackCoordinator(
    ...
    fun launchPlay(
        ...
        playJob = scope.launch {
            ...
            try {
                ...
                // Caller drives UI for cancellation paths.
            } finally {
                if (token == playRequestToken) {
                    playJob = null
                    onFinally()
    ...
    fun stopPlaybackStateObserver() {
        playbackStateJob?.cancel()
        playbackStateJob = null
    }

    suspend fun refreshPlaybackState(onPlaybackState: (Int) -> Unit) {
        onPlaybackState(queryPlaybackState())
    }

    fun close() {
        stopPlaybackStateObserver()
        stopPlayback()
        player.close()
    }
