...
internal sealed interface ContentEntryAction {
    data object OpenDailyRecommendedSongs : ContentEntryAction
...
internal fun resolveContentEntryLaunch(
    ...
    return when (action) {
        ContentEntryAction.OpenDailyRecommendedSongs -> ContentEntryLaunch(
            intent = DailyRecommendedSongsActivity.createIntent(context)
        )
